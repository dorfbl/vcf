#main.tf for VCF 5.1 Bringup - last updated on 12-1-2023
...
#	var.ntp2
  ...
  network {
    subnet = var.vm_management_subnet
    vlan_id = var.vm_management_vlan
    mtu = var.vm_management_mtu
    network_type = "VM_MANAGEMENT"
    gateway = var.vm_management_gw
	port_group_key = var.vm_management_portgroup
  }
  network {
